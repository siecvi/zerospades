...
namespace spades {
	...
	class StartupScreenConfigViewModel : spades::ui::ListViewModel {
		...
		void Filter(string text) {
			...
			for (uint i = 0, count = items.length; i < count; i++) {
				StartupScreenConfigItemEditor@ editor = cast<StartupScreenConfigItemEditor>(items[i]);
				if (editor is null)
					continue;
	...
	class StartupScreenConfigCheckItemEditor :
		...
		void set_Bounds(AABB2 v) {
			...
			Vector2 size = this.Size;
			float h = 24.0F;
			button.Bounds = AABB2(0.0F, 0.0F, size.x, h);
		}
	}
	...
	class StartupScreenConfigSliderItemEditor :
		...
		void set_Bounds(AABB2 v) {
			...
			Vector2 size = this.Size;
			float h = 24.0F;
			slider.Bounds = AABB2(100.0F, 2.0F, size.x - 180.0F, h - 4.0F);
			valueLabel.Bounds = AABB2(size.x - 80.0F, 0.0F, 80.0F, h);
		}
	}
	...
	class StartupScreenConfigFieldItemEditor :
		...
		void set_Bounds(AABB2 v) {
			...
			Vector2 size = this.Size;
			float h = 24.0F;
			field.Bounds = AABB2(240.0F, 0.0F, size.x - 240.0F, h);
		}
	}
	...
	class StartupScreenComplexConfigDialog : spades::ui::UIElement {
		...
		StartupScreenComplexConfigDialog(spades::ui::UIManager@ manager, StartupScreenComplexConfig@ config) {
...

			{
				...
				cfg.Bounds = AABB2(10.0F, ContentsTop + 30.0F, mainWidth - 20.0F, ContentsHeight - 60.0F);
				AddChild(cfg);
				@configView = cfg;
			}

			{
				...
				button.Bounds = AABB2(size.x - 160.0F, ContentsTop + ContentsHeight - 30.0F, 150.0F, 30.0F);
		...
		void Render() {
			...
			r.ColorNP = Vector4(0.0F, 0.0F, 0.0F, 0.8F);
			r.DrawImage(null, AABB2(pos.x, pos.y, size.x, ContentsTop - 15.0F));
			r.DrawImage(null, AABB2(pos.x, ContentsTop + ContentsHeight + 15.0F, size.x, ContentsTop - 15.0F));

			r.ColorNP = Vector4(0.0F, 0.0F, 0.0F, 0.95F);
			r.DrawImage(null, AABB2(pos.x, ContentsTop - 15.0F, size.x, ContentsHeight + 30.0F));
			...
			Font@ font = Font;
			r.ColorNP = Vector4(0.8F, 0.8F, 0.8F, 1.0F);
			r.DrawImage(null, AABB2(pos.x, pos.y + ContentsTop, size.x, 20.0F));
			font.Draw(Title, Vector2(pos.x + 10.0F, pos.y + ContentsTop), 1.0F, Vector4(0.0F, 0.0F, 0.0F, 1.0F));
