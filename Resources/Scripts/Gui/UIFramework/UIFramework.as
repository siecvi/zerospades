	...
	namespace ui {
		...
		class UIManager {
			...
			UIManager(Renderer@ renderer, AudioDevice@ audioDevice) {
				...
				@DefaultCursor = Cursor(this, renderer.RegisterImage("Gfx/UI/Cursor.png"), Vector2(8.0F, 8.0F));
		...
		class KeyRepeatManager {
			...
			void RunFrame(float dt) {
				...
				if (nextDelay < 0.0F) {
					handler(lastKey);
					nextDelay = Max(nextDelay + 0.06F, 0.0F);
				}
			}
		}
		...
		class Timer {
			...
			void RunFrame(float dt) {
				...
				if (nextDelay < 0.0F) {
					OnTick();
					if (AutoReset) {
						nextDelay = Max(nextDelay + Interval, 0.0F);
					} else {
						Stop();
					}
				}
		...
		class UIElement {
			...
			AABB2 TextInputRect {
				get { return AABB2(0, 0, Size.x, Size.y); }
			}
		...
		class Cursor {
			...
			void Render(Vector2 pos) {
				...
				r.ColorOpaque = HSV(Max(manager.Time * 0.1F, 0.0F), 1.0F, 1.0F);
