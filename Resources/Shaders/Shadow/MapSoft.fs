...
vec3 MapSoft_BlockSample(vec2 sample, float depth, float shiftedDepth) {
	const float factor = 1.0 / 512.0;
	float val = texture2D(mapShadowTexture, sample.xy * factor).w;
	float distance = shiftedDepth - val;
	float weight = step(0.0, distance);
	weight = clamp(distance*255.0 - 1.0, 0.0, 1.0);
	return vec3(distance, 1.0, step(0.0, shiftedDepth - val)) * weight;
}
...
float EvaluateMapShadow() {
	...
	vec2 fracPosHS = fracPos - 0.5;				// [-0.5, 0.5]
	...
	float depthShifted = depth + sampShift.y / 255.0;
	...
	distWeighted1 = mix(distWeighted1, samp2, fracPosHSAbs.x);

	vec3 distWeighted2 = samp3;
	distWeighted2 = mix(distWeighted2, samp4, fracPosHSAbs.x);

	vec3 distWeighted3 = mix(distWeighted1, distWeighted2, fracPosHSAbs.y);
	distWeighted3.x /= distWeighted3.y + 1.E-10;

	float distance = distWeighted3.x;

	// blurred shadow sampling
	float blur = distance * 4.0;
	blur = max(blur, 1.E-10); // avoid zero division

	vec2 blurWeight = 0.5 - (0.5 - fracPosHSAbs) / blur;
	blurWeight = max(blurWeight, 0.0);

	float val1 = mix(samp1.z, samp2.z, blurWeight.x);
	float val2 = mix(samp3.z, samp4.z, blurWeight.x);
	float val = 1.0 - mix(val1, val2, blurWeight.y);

	// --- sharp shadow
	vec4 sharpCol = texture2D(mapShadowTexture, floor(mapShadowCoord.xy) / 512.0);
	float sharpVal = sharpCol.w;

	// side shadow?
	if (sharpCol.x > 0.499)
		sharpVal -= fract(mapShadowCoord.y) / 255.0;
