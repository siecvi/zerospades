...
namespace spades {
	namespace client {
		...
		class RendererRegistrar : public ScriptObjectRegistrar {
			...
			static void RenderModel(IModel* model, const ModelRenderParam& param, IRenderer* r) {
				try {
					if (!model) {
						SPInvalidArgument("model");
					}
			...
			static void AddDebugLine(const Vector3& a, const Vector3& b, const Vector4& color,
			                         IRenderer* r) {
				try {
			...
			static void AddSprite(IImage* img, const Vector3& center, float rad, float rot,
			                      IRenderer* r) {
				try {
					if (!img) {
			...
			static void AddLongSprite(IImage* img, const Vector3& p1, const Vector3& p2, float rad,
			                          IRenderer* r) {
				try {
					if (!img) {
						SPInvalidArgument("img");
			...
			static void MultiplyScreenColor(const Vector3& v, IRenderer* r) {
				try {
			...
			static void SetColor(const Vector4& v, IRenderer* r) {
				try {
					return r->SetColor(v);
				} catch (const std::exception& ex) {
					ScriptContextUtils().SetNativeException(ex);
			...
			static void SetColorOpaque(const Vector3& v, IRenderer* r) {
				try {
			...
			static void SetColorAlphaPremultiplied(const Vector4& v, IRenderer* r) {
				try {
			...
			static void SetColorAlphaNonPremultiplied(const Vector4& v, IRenderer* r) {
			...
			static void SetFogColor(const Vector3& v, IRenderer* r) {
				try {
			...
			static void ModelRenderParamFactory(ModelRenderParam* p) { new (p) ModelRenderParam(); }
			static void DynamicLightParamFactory(DynamicLightParam* p) {
				new (p) DynamicLightParam();
			}
			static void DynamicLightParamFactory2(const DynamicLightParam& other,
			                                      DynamicLightParam* p) {
		...
		public:
			RendererRegistrar() : ScriptObjectRegistrar("Renderer") {}
			virtual void Register(ScriptManager* manager, Phase phase) {
				...
				switch (phase) {
					case PhaseObjectType:
						r = eng->RegisterObjectType("Renderer", 0, asOBJ_REF);
						...
						manager->CheckError(r);
						r = eng->RegisterObjectType("DynamicLightParam", sizeof(DynamicLightParam),
						                            asOBJ_VALUE | asOBJ_APP_CLASS_CDAK);
						manager->CheckError(r);
						r = eng->RegisterObjectType("SceneDefinition", sizeof(SceneDefinition),
						                            asOBJ_VALUE | asOBJ_POD | asOBJ_APP_CLASS_CDAK);
						manager->CheckError(r);
					...
					case PhaseObjectMember:
						...
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("ModelRenderParam", "Vector3 customColor",
						                                asOFFSET(ModelRenderParam, customColor));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("ModelRenderParam", "bool depthHack",
						...
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("DynamicLightParam", "Vector3 origin",
						                                asOFFSET(DynamicLightParam, origin));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("DynamicLightParam", "Vector3 point2",
						                                asOFFSET(DynamicLightParam, point2));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("DynamicLightParam", "float radius",
						                                asOFFSET(DynamicLightParam, radius));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("DynamicLightParam", "Vector3 color",
						                                asOFFSET(DynamicLightParam, color));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("DynamicLightParam", "Vector3 spotAxisX",
						                                asOFFSET(DynamicLightParam, spotAxis[0]));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("DynamicLightParam", "Vector3 spotAxisY",
						                                asOFFSET(DynamicLightParam, spotAxis[1]));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("DynamicLightParam", "Vector3 spotAxisZ",
						                                asOFFSET(DynamicLightParam, spotAxis[2]));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("DynamicLightParam", "Image@ image",
						                                asOFFSET(DynamicLightParam, image));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("DynamicLightParam", "float spotAngle",
						                                asOFFSET(DynamicLightParam, spotAngle));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("DynamicLightParam", "bool useLensFlare",
						...
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("SceneDefinition", "int viewportTop",
						                                asOFFSET(SceneDefinition, viewportTop));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("SceneDefinition", "int viewportWidth",
						                                asOFFSET(SceneDefinition, viewportWidth));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("SceneDefinition", "int viewportHeight",
						                                asOFFSET(SceneDefinition, viewportHeight));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("SceneDefinition", "float fovX",
						                                asOFFSET(SceneDefinition, fovX));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("SceneDefinition", "float fovY",
						                                asOFFSET(SceneDefinition, fovY));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("SceneDefinition", "Vector3 viewOrigin",
						                                asOFFSET(SceneDefinition, viewOrigin));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("SceneDefinition", "Vector3 viewAxisX",
						                                asOFFSET(SceneDefinition, viewAxis[0]));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("SceneDefinition", "Vector3 viewAxisY",
						                                asOFFSET(SceneDefinition, viewAxis[1]));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("SceneDefinition", "Vector3 viewAxisZ",
						                                asOFFSET(SceneDefinition, viewAxis[2]));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("SceneDefinition", "float zNear",
						                                asOFFSET(SceneDefinition, zNear));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("SceneDefinition", "float zFar",
						...
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("SceneDefinition", "uint time",
						                                asOFFSET(SceneDefinition, time));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("SceneDefinition", "bool denyCameraBlur",
						                                asOFFSET(SceneDefinition, denyCameraBlur));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("SceneDefinition", "float blurVignette",
						                                asOFFSET(SceneDefinition, blurVignette));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("SceneDefinition", "float radialBlur",
						                                asOFFSET(SceneDefinition, radialBlur));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("SceneDefinition", "float globalBlur",
