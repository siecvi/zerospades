	...
	namespace client {
		...
		class GameMapRegistrar : public ScriptObjectRegistrar {
			static GameMap* Factory(int w, int h, int d) {
				if (w != GameMap::DefaultWidth || h != GameMap::DefaultHeight ||
				    d != GameMap::DefaultDepth) {
					asGetActiveContext()->SetException(
					  "Currently, non-default GameMap dimensions aren't supported.");
					return nullptr;
				}
				try {
					return new GameMap();
			...
			static GameMap* LoadFactory(const std::string& fn) {
				try {
					std::unique_ptr<spades::IStream> stream{
					  FileManager::OpenForReading(fn.c_str())};
			...
			static uint32_t GetColor(int x, int y, int z, GameMap* m) {
				if (x < 0 || y < 0 || x >= m->Width() || y >= m->Height() || z < 0 ||
				    z >= m->Depth()) {
					asGetActiveContext()->SetException(
					  "Attempted to access a voxel outside the valid range.");
					return 0;
				}
				return m->GetColor(x, y, z);
			}

			static bool IsSolid(int x, int y, int z, GameMap* m) {
				if (x < 0 || y < 0 || x >= m->Width() || y >= m->Height() || z < 0 ||
				    z >= m->Depth()) {
					asGetActiveContext()->SetException(
					  "Attempted to access a voxel outside the valid range.");
			...
			static uint32_t GetColorWrapped(int x, int y, int z, GameMap* m) {
				return m->GetColorWrapped(x, y, z);
			}

			static bool IsSolidWrapped(int x, int y, int z, GameMap* m) {
				return m->IsSolidWrapped(x, y, z);
			}
			static void SetAir(int x, int y, int z, GameMap* m) {
				if (x < 0 || y < 0 || x >= m->Width() || y >= m->Height() || z < 0 ||
				    z >= m->Depth()) {
					asGetActiveContext()->SetException(
					  "Attempted to access a voxel outside the valid range.");
			...
			static void SetSolid(int x, int y, int z, uint32_t col, GameMap* m) {
				if (x < 0 || y < 0 || x >= m->Width() || y >= m->Height() || z < 0 ||
				    z >= m->Depth()) {
					asGetActiveContext()->SetException(
					  "Attempted to access a voxel outside the valid range.");
			...
			static GameMap::RayCastResult CastRay(const Vector3& v0, const Vector3& dir,
			                                      int maxSteps, GameMap* m) {
				return m->CastRay2(v0, dir, maxSteps);
			}

		public:
			GameMapRegistrar() : ScriptObjectRegistrar("GameMap") {}
			...
			virtual void Register(ScriptManager* manager, Phase phase) {
				...
				switch (phase) {
					case PhaseObjectType:
						r = eng->RegisterObjectType("GameMap", 0, asOBJ_REF);
						manager->CheckError(r);
						r = eng->RegisterObjectType("GameMapRayCastResult",
						                            sizeof(GameMap::RayCastResult),
					...
					case PhaseObjectMember:
						...
						manager->CheckError(r);
						r =
						  eng->RegisterObjectProperty("GameMapRayCastResult", "bool startSolid",
						                              asOFFSET(GameMap::RayCastResult, startSolid));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("GameMapRayCastResult", "Vector3 hitPos",
						                                asOFFSET(GameMap::RayCastResult, hitPos));
						manager->CheckError(r);
						r =
						  eng->RegisterObjectProperty("GameMapRayCastResult", "IntVector3 hitBlock",
						                              asOFFSET(GameMap::RayCastResult, hitBlock));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("GameMapRayCastResult", "IntVector3 normal",
