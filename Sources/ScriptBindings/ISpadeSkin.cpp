...
namespace spades {
	namespace client {
		ScriptISpadeSkin::ScriptISpadeSkin(asIScriptObject* obj) : obj(obj) {}

		void ScriptISpadeSkin::SetActionType(SpadeActionType act) {
			SPADES_MARK_FUNCTION_DEBUG();
			static ScriptFunction func("ISpadeSkin", "void set_ActionType(SpadeActionType)");
		...
		void ScriptISpadeSkin::SetActionProgress(float v) {
			SPADES_MARK_FUNCTION_DEBUG();
			static ScriptFunction func("ISpadeSkin", "void set_ActionProgress(float)");
			ScriptContextHandle ctx = func.Prepare();
			int r;
			r = ctx->SetObject((void*)obj);
			ScriptManager::CheckError(r);
		...
		public:
			ISpadeSkinRegistrar() : ScriptObjectRegistrar("ISpadeSkin") {}
			virtual void Register(ScriptManager* manager, Phase phase) {
				...
				switch (phase) {
					...
					case PhaseObjectMember:
						r = eng->RegisterInterfaceMethod("ISpadeSkin", "void set_ActionType(SpadeActionType)");
						manager->CheckError(r);
						r = eng->RegisterInterfaceMethod("ISpadeSkin", "void set_ActionProgress(float)");
						manager->CheckError(r);
						break;
					default: break;
				}
