...
namespace spades {
	...
	class VoxelModelRegistrar : public ScriptObjectRegistrar {
		...
		static uint64_t GetSolidBits(int x, int y, VoxelModel* m) {
		...
		static uint32_t GetColor(int x, int y, int z, VoxelModel* m) {
			if (x < 0 || y < 0 || x >= m->GetWidth() || y >= m->GetHeight() || z < 0 ||
			    z >= m->GetDepth()) {
				asGetActiveContext()->SetException(
				  "Attempted to access a voxel outside the valid range.");
				return 0;
		...
		static bool IsSolid(int x, int y, int z, VoxelModel* m) {
			if (x < 0 || y < 0 || x >= m->GetWidth() || y >= m->GetHeight() || z < 0 ||
			    z >= m->GetDepth()) {
				asGetActiveContext()->SetException(
				  "Attempted to access a voxel outside the valid range.");
		...
		static void SetAir(int x, int y, int z, VoxelModel* m) {
			if (x < 0 || y < 0 || x >= m->GetWidth() || y >= m->GetHeight() || z < 0 ||
			    z >= m->GetDepth()) {
				asGetActiveContext()->SetException(
				  "Attempted to access a voxel outside the valid range.");
		...
		static void SetSolid(int x, int y, int z, uint32_t col, VoxelModel* m) {
			if (x < 0 || y < 0 || x >= m->GetWidth() || y >= m->GetHeight() || z < 0 ||
			    z >= m->GetDepth()) {
				asGetActiveContext()->SetException(
				  "Attempted to access a voxel outside the valid range.");
		...
		virtual void Register(ScriptManager* manager, Phase phase) {
			...
			switch (phase) {
				case PhaseObjectType:
					r = eng->RegisterObjectType("VoxelModel", 0, asOBJ_REF);
