...
namespace spades {
	namespace client {
		...
		class AudioDeviceRegistrar : public ScriptObjectRegistrar {
			static void AudioParamFactory(AudioParam* p) { new (p) AudioParam(); }
		...
		public:
			AudioDeviceRegistrar() : ScriptObjectRegistrar("AudioDevice") {}
			virtual void Register(ScriptManager* manager, Phase phase) {
				...
				switch (phase) {
					case PhaseObjectType:
						r = eng->RegisterObjectType("AudioDevice", 0, asOBJ_REF);
						manager->CheckError(r);
						r = eng->RegisterObjectType("AudioParam", sizeof(AudioParam),
					...
					case PhaseObjectMember:
						...
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("AudioParam", "float pitch",
						                                asOFFSET(AudioParam, pitch));
						manager->CheckError(r);
						r = eng->RegisterObjectProperty("AudioParam", "float referenceDistance",
