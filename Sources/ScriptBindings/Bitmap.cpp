...
namespace spades {
	class BitmapRegistrar : public ScriptObjectRegistrar {
		static Bitmap* Factory(int w, int h) { return new Bitmap(w, h); }
		...
		static bool Save(const std::string& str, Bitmap* bmp) {
			try {
				bmp->Save(str);
			} catch (const std::exception& ex) {
				ScriptContextUtils().SetNativeException(ex);
		...
		static uint32_t GetPixel(int x, int y, Bitmap* bmp) {
			if (x < 0 || y < 0 || x >= bmp->GetWidth() || y >= bmp->GetHeight()) {
				asGetActiveContext()->SetException(
				  "Attempted to fetch a pixel outside the valid range.");
				return 0;
			}
			return bmp->GetPixel(x, y);
		}

		static void SetPixel(int x, int y, uint32_t val, Bitmap* bmp) {
			if (x < 0 || y < 0 || x >= bmp->GetWidth() || y >= bmp->GetHeight()) {
				asGetActiveContext()->SetException(
				  "Attempted to write a pixel outside the valid range.");
				return;
			}
	...
	public:
		BitmapRegistrar() : ScriptObjectRegistrar("Bitmap") {}
		...
		virtual void Register(ScriptManager* manager, Phase phase) {
			...
			switch (phase) {
				case PhaseObjectType:
					r = eng->RegisterObjectType("Bitmap", 0, asOBJ_REF);
