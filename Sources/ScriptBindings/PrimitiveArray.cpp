...
namespace spades {
	...
	public:
		...
		PrimitiveArray(asUINT initialSize = 0) { inner.resize(initialSize); }
		...
		PrimitiveArray(void* initList) {
			asUINT length = *reinterpret_cast<const asUINT*>(initList);
			inner.resize(length);
			memcpy(inner.data(), reinterpret_cast<const asUINT*>(initList) + 1,
			       inner.size() * sizeof(T));
		}
		...
		void InsertLast(const T& val) { inner.push_back(val); }
		void RemoveLast() {
			if (inner.empty()) {
		...
		asUINT GetSize() const { return (asUINT)inner.size(); }
		...
		void SortDesc(asUINT index, asUINT count) {
			...
			struct Sorter {
				static bool Sort(const T& a, const T& b) { return a > b; }
			};
			std::sort(inner.begin() + index, inner.begin() + index + count, Sorter::Sort);
		}
		void SortAsc() { SortAsc(0, GetSize()); }
		void SortDesc() { SortDesc(0, GetSize()); }

		void Reverse() { std::reverse(inner.begin(), inner.end()); }
		...
		int Find(asUINT ind, const T& val) const {
			...
			typename std::vector<T>::const_iterator it =
			  std::find(inner.begin() + ind, inner.end(), val);
			if (it == inner.end())
				return -1;
			return static_cast<int>(it - inner.begin());
		}
	...
	template <typename T> class PrimitiveArrayRegistrar : public ScriptObjectRegistrar {
		std::string typeName;
		std::string arrayTypeName;
		std::string tmpStr[4];
		int tmpId;
		typedef PrimitiveArray<T> ArrayType;
		const char* TN() { return typeName.c_str(); }
		const char* ATN() { return arrayTypeName.c_str(); }
	...
	public:
		PrimitiveArrayRegistrar(const char* nam)
		    : ScriptObjectRegistrar(nam), typeName(nam), tmpId(0) {
		...
		virtual void Register(ScriptManager* manager, Phase phase) {
			...
			switch (phase) {
				case PhaseObjectType:
					r = eng->RegisterObjectType(ATN(), 0, asOBJ_REF | asOBJ_GC);
