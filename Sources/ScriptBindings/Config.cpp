...
namespace spades {
	...
	namespace {
		...
		std::unordered_map<std::string, const SettingItemDescriptor*> settingItemDescriptors;

		const SettingItemDescriptor* MakeSettingItemDescriptor(const std::string& name,
	...
	public:
		ConfigRegistrar() : ScriptObjectRegistrar("Config") {}
		...
		public:
			...
			ConfigItem(const std::string& name) : handle(name, nullptr) {}

			static ConfigItem* Construct(const std::string& name, const std::string& defaultValue) {
				return new ConfigItem(name, defaultValue);
			}
			static ConfigItem* Construct(const std::string& name) { return new ConfigItem(name); }
			...
			void SetValue(float v) { handle = v; }
			void SetValue(int v) { handle = v; }
			void SetValue(const std::string& v) { handle = v; }
			float GetFloatValue() { return (float)handle; }
			int GetIntValue() { return (int)handle; }
			bool GetBoolValue() { return (bool)handle; }
			std::string GetStringValue() { return (std::string)handle; }
			std::string GetDefaultValue() { return handle.GetDescriptor().defaultValue; }
			bool IsUnknown() { return handle.IsUnknown(); }
		};
		...
		virtual void Register(ScriptManager* manager, Phase phase) {
			...
			switch (phase) {
				case PhaseObjectType:
					r = eng->RegisterObjectType("ConfigItem", 0, asOBJ_REF);
				...
				case PhaseObjectMember:
					...
					r = eng->RegisterObjectMethod(
					  ...
					  asCALL_THISCALL);
					manager->CheckError(r);
					r = eng->RegisterObjectMethod("ConfigItem", "void set_IntValue(int)",
					                              asMETHODPR(ConfigItem, SetValue, (int), void),
					                              asCALL_THISCALL);
					manager->CheckError(r);
					r = eng->RegisterObjectMethod("ConfigItem", "void set_FloatValue(float)",
					                              asMETHODPR(ConfigItem, SetValue, (float), void),
					                              asCALL_THISCALL);
					manager->CheckError(r);
					r = eng->RegisterObjectMethod(
					  "ConfigItem", "void set_StringValue(const string& in)",
					  asMETHODPR(ConfigItem, SetValue, (const std::string&), void),
					  asCALL_THISCALL);
					manager->CheckError(r);
					r =
					  eng->RegisterObjectMethod("ConfigItem", "int get_IntValue()",
					                            asMETHOD(ConfigItem, GetIntValue), asCALL_THISCALL);
					manager->CheckError(r);
					r = eng->RegisterObjectMethod("ConfigItem", "bool get_BoolValue()",
					                              asMETHOD(ConfigItem, GetBoolValue),
					                              asCALL_THISCALL);
					manager->CheckError(r);
					r = eng->RegisterObjectMethod("ConfigItem", "float get_FloatValue()",
					                              asMETHOD(ConfigItem, GetFloatValue),
					                              asCALL_THISCALL);
					manager->CheckError(r);
					r = eng->RegisterObjectMethod("ConfigItem", "string get_StringValue()",
					                              asMETHOD(ConfigItem, GetStringValue),
					                              asCALL_THISCALL);
					manager->CheckError(r);
					r = eng->RegisterObjectMethod("ConfigItem", "string get_DefaultValue()",
					                              asMETHOD(ConfigItem, GetDefaultValue),
					                              asCALL_THISCALL);
					manager->CheckError(r);
					r = eng->RegisterObjectMethod("ConfigItem", "bool get_IsUnknown()",
