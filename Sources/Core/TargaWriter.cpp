...
static const char tga_id[] = "\0\0\0\0" /* extension area offset */
...
static tga_result tga_write_row_RLE(spades::IStream* fp, const tga_image* src, const uint8_t* row);
typedef enum { RAW, RLE } packet_type;
static packet_type rle_packet_type(const uint8_t* row, const uint16_t pos, const uint16_t width,
                                   const uint16_t bpp);
static uint8_t rle_packet_len(const uint8_t* row, const uint16_t pos, const uint16_t width,
                              const uint16_t bpp, const packet_type type);
...
const char* tga_error(const tga_result errcode) {
	switch (errcode) {
		case TGA_NOERR: return "no error";
		case TGAERR_FOPEN: return "error opening file";
		case TGAERR_EOF: return "premature end of file";
		case TGAERR_WRITE: return "error writing to file";
		case TGAERR_CMAP_TYPE: return "invalid color map type";
		case TGAERR_IMG_TYPE: return "invalid image type";
		case TGAERR_NO_IMG: return "no image data included";
		case TGAERR_CMAP_MISSING: return "color-mapped image without color map";
		case TGAERR_CMAP_PRESENT: return "non-color-mapped image with extraneous color map";
		case TGAERR_CMAP_LENGTH: return "color map has zero length";
		case TGAERR_CMAP_DEPTH: return "invalid color map depth";
		case TGAERR_ZERO_SIZE: return "the image dimensions are zero";
		case TGAERR_PIXEL_DEPTH: return "invalid pixel depth";
		case TGAERR_NO_MEM: return "out of memory";
		case TGAERR_NOT_CMAP: return "image is not color mapped";
		case TGAERR_RLE: return "RLE data is corrupt";
		case TGAERR_INDEX_RANGE: return "color map index out of range";
		case TGAERR_MONO: return "image is mono";
...
#define READ(destptr, size)                                                                        \
	if (fp->Read(destptr, size) < uint8_t(size))                                                   \
	BARF(TGAERR_EOF)
...
#define READ16(dest)                                                                               \
	...
	if (dest->image_type != TGA_IMAGE_TYPE_COLORMAP && dest->image_type != TGA_IMAGE_TYPE_BGR &&
	    dest->image_type != TGA_IMAGE_TYPE_MONO &&
	    dest->image_type != TGA_IMAGE_TYPE_COLORMAP_RLE &&
	    dest->image_type != TGA_IMAGE_TYPE_BGR_RLE && dest->image_type != TGA_IMAGE_TYPE_MONO_RLE)
	...
	if (dest->color_map_type == TGA_COLOR_MAP_PRESENT) {
		dest->color_map_data = (uint8_t*)malloc((dest->color_map_origin + dest->color_map_length) *
		                                        dest->color_map_depth / 8);
		if (dest->color_map_data == NULL)
			BARF(TGAERR_NO_MEM);
		READ(dest->color_map_data + (dest->color_map_origin * dest->color_map_depth / 8),
...
static tga_result tga_write_row_RLE(spades::IStream* fp, const tga_image* src, const uint8_t* row) {
#define WRITE(src, size) fp->Write(src, size)
 ...
 */
tga_result tga_write_to_FILE(spades::IStream* fp, const tga_image* src) {
#define WRITE(srcptr, size) fp->Write(srcptr, size)
...
#define WRITE16(src)                                                                               \
	...
	if (src->image_type != TGA_IMAGE_TYPE_COLORMAP && src->image_type != TGA_IMAGE_TYPE_BGR &&
	    src->image_type != TGA_IMAGE_TYPE_MONO && src->image_type != TGA_IMAGE_TYPE_COLORMAP_RLE &&
	    src->image_type != TGA_IMAGE_TYPE_BGR_RLE && src->image_type != TGA_IMAGE_TYPE_MONO_RLE)
	...
	if (src->color_map_type == TGA_COLOR_MAP_PRESENT)
		WRITE(src->color_map_data + (src->color_map_origin * src->color_map_depth / 8),
	...
	if (tga_is_rle(src)) {
		...
		for (row = 0; row < src->height; row++) {
			tga_result result =
			  tga_write_row_RLE(fp, src, src->image_data + row * src->width * src->pixel_depth / 8);
			if (result != TGA_NOERR)
				return result;
		}
	} else {
...
#undef FIVE_BITS
		...
		default: return TGAERR_PIXEL_DEPTH;
	}
	return TGA_NOERR;
}
...
tga_result tga_convert_depth(tga_image* img, const uint8_t bits) {
	...
	if (src_size > dest_size) {
		...
		/* convert forwards */
		dest = img->image_data;
		for (src = img->image_data; src < img->image_data + img->width * img->height * src_bpp;
		     src += src_bpp) {
			uint8_t r, g, b, a;
...
tga_result tga_swap_red_blue(tga_image* img) {
	...
	for (ptr = img->image_data; ptr < img->image_data + (img->width * img->height - 1) * bpp;
	     ptr += bpp) {
		uint8_t r, g, b, a;
