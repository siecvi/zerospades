...
static const uint16_t cp437map[256] = {
  // graphic symbols
  0x0000, 0x263a, 0x263b, 0x2665, 0x2666, 0x2663, 0x2660, 0x2022, 0x25d8, 0x25cb, 0x000a, 0x2642,
  0x2640, 0x000d, 0x266b, 0x263c, 0x25ba, 0x25c4, 0x2195, 0x203c, 0x00b6, 0x00a7, 0x25ac, 0x21a8,
...
namespace spades {
	...
	namespace {
		struct ReverseMap {
			std::unordered_map<uint32_t, char> mp;
			ReverseMap() {
				for (int i = 0; i < 256; i++)
		...
		ReverseMap const g_reverse;
	} // namespace

	char CP437::EncodeChar(uint32_t unicode, char fb) {
		auto it = g_reverse.mp.find(unicode);
		if (it == g_reverse.mp.end()) {
			return fb;
		} else {
	...
	std::uint32_t CP437::DecodeChar(char c) {
		...
		SPAssert(g_reverse.mp.find(r)->second == c);
		return r;
	}

	std::string CP437::Encode(const std::string& str, char fb) {
		size_t idx = 0;
		std::string outp;
		...
		while (idx < str.size()) {
			...
			idx += outNumBytes;
			outp.push_back(EncodeChar(cp, fb));
		}
		return outp;
	}
