...
typedef unsigned (*png_write_callback_t)(void* input, size_t size, size_t numel,
                                         void* user_pointer);
typedef unsigned (*png_read_callback_t)(void* output, size_t size, size_t numel,
                                        void* user_pointer);
...
/*
    ...
    This function initializes pnglite. The parameters can be used to set your own memory allocation
   routines following these formats:

    > void* (*custom_alloc)(size_t s)
    > void (*custom_free)(void* p)
    Parameters:
        pngalloc - Pointer to custom allocation routine. If 0 is passed, malloc from libc will be
   used. pngfree - Pointer to custom free routine. If 0 is passed, free from libc will be used.
...
/*
    ...
    This function is used to open a png file with the internal file IO system. This function should
   be used instead of png_open if no custom read function is used.
...
/*
    ...
    This function reads or writes a png from/to the specified callback. The callbacks should be of
   the format:
    ...
    Only one callback has to be specified. The read callback in case of PNG reading, otherwise the
   write callback.
    ...
    Reading:
    The callback will be called each time pnglite needs more data. The callback should read as much
   data as requested, or return 0. This should always be possible if the PNG is sane.	If the
   output-buffer is a null-pointer the callback should only skip ahead the specified number of
   elements. If the callback is a null-pointer the user_pointer will be treated as a file pointer
   (use png_open_file instead).
...
/*
    ...
    This function decodes the opened png file and stores the result in data. data should be big
   enough to hold the decoded png. Required size will be:
...
int png_set_data(png_t* png, unsigned width, unsigned height, char depth, int color,
                 unsigned char* data);

/*
    Function: png_close_file

    Closes an open png file pointer. Should only be used when the png has been opened with
   png_open_file.
