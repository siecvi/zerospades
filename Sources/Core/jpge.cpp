...
// v1.02, Apr. 6, 2011 - Removed 2x2 ordered dither in H2V1 chroma subsampling method
// load_block_16_8_8(). (The rounding factor was 2, when it should have been 1. Either way, it
// wasn't helping.) v1.03, Apr. 16, 2011 - Added support for optimized Huffman code tables,
// optimized dynamic memory allocation down to only 1 alloc.
//                        Also from Alex Evans: Added RGBA support, linear memory allocator (no
//                        longer needed in v1.03).
// v1.04, May. 19, 2012: Forgot to set m_pFile ptr to NULL in cfile_stream::close(). Thanks to Owen
// Kaluza for reporting this bug.
//                       Code tweaks to fix VS2008 static code analysis warnings (all looked
//                       harmless). Code review revealed method load_block_16_8_8() (used for the
//                       non-default H2V1 sampling mode to downsample chroma) somehow didn't get the
//                       rounding factor fix from v1.02.
...
namespace jpge {
	...
	static uint8 s_zag[64] = {0,  1,  8,  16, 9,  2,  3,  10, 17, 24, 32, 25, 18, 11, 4,  5,
	...
	static void RGB_to_Y(uint8* pDst, const uint8* pSrc, int num_pixels) {
		for (; num_pixels; pDst++, pSrc += 3, num_pixels--) {
			pDst[0] =
			  static_cast<uint8>((pSrc[0] * YR + pSrc[1] * YG + pSrc[2] * YB + 32768) >> 16);
		}
	}
...
#define DCT1D(s0, s1, s2, s3, s4, s5, s6, s7)                                                      \
	...
	// calculate_minimum_redundancy() originally written by: Alistair Moffat, alistair@cs.mu.oz.au,
	...
	void jpeg_encoder::optimize_huffman_table(int table_num, int table_len) {
		...
		const uint JPGE_CODE_SIZE_LIMIT =
		  16; // the maximum possible size of a JPEG Huffman code (valid range is [9,16] - 9 vs. 8
		      // because of the dummy symbol)
		...
		// Compute the m_huff_val array, which contains the symbol indices sorted by code size
		// (smallest to largest).
	...
	// Compute the actual canonical Huffman codes/code sizes given the JPEG huff bits and val
	// arrays.
	void jpeg_encoder::compute_huffman_table(uint* codes, uint8* code_sizes, uint8* bits,
	                                         uint8* val) {
		int i, l, last_p, si;
		uint8 huff_size[257];
		uint huff_code[257];
	...
	bool jpeg_encoder::jpg_open(int p_x_res, int p_y_res, int src_channels) {
		...
		compute_quant_table(m_quantization_tables[1], m_params.m_no_chroma_discrim_flag
		                                                ? s_std_lum_quant
		                                                : s_std_croma_quant);
	...
	void jpeg_encoder::load_block_16_8(int x, int c) {
		...
		for (int i = 0; i < 16; i += 2, pDst += 8) {
			...
			pDst[5] =
			  ((pSrc1[10 * 3] + pSrc1[11 * 3] + pSrc2[10 * 3] + pSrc2[11 * 3] + b) >> 2) - 128;
			pDst[6] =
			  ((pSrc1[12 * 3] + pSrc1[13 * 3] + pSrc2[12 * 3] + pSrc2[13 * 3] + a) >> 2) - 128;
			pDst[7] =
			  ((pSrc1[14 * 3] + pSrc1[15 * 3] + pSrc2[14 * 3] + pSrc2[15 * 3] + b) >> 2) - 128;
			int temp = a;
			a = b;
			b = temp;
		}
...
#include <stdio.h>
	...
	public:
		...
		uint get_size() const { return m_pFile ? (uint)ftell(m_pFile) : 0; }
	};
	...
	bool compress_image_to_jpeg_file_in_memory(void* pDstBuf, int& buf_size, int width, int height,
