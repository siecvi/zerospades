...
static int png_process_chunk(png_t* png) {
	...
	if (type == *(unsigned int*)"IDAT") /* if we found an idat, all other idats should be followed
...
static void png_filter_up(int stride, unsigned char* in, unsigned char* out,
...
static void png_filter_average(int stride, unsigned char* in, unsigned char* out,
                               unsigned char* prev_line, int len) {
	int i;
	unsigned char a = 0;
	unsigned char b = 0;
...
static void png_filter_paeth(int stride, unsigned char* in, unsigned char* out,
                             unsigned char* prev_line, int len) {
	int i;
	unsigned char a;
	unsigned char b;
...
static int png_unfilter(png_t* png, unsigned char* data) {
	...
	while (pos < png->png_datalen) {
		...
		switch (filter) {
			case 0: /* none */ memcpy(data + outpos, filtered + pos, png->width * stride); break;
			case 1: /* sub */
			...
			case 2: /* up */
				if (outpos)
					png_filter_up(stride, filtered + pos, data + outpos,
					              data + outpos - (png->width * stride), png->width * stride);
				else
					png_filter_up(stride, filtered + pos, data + outpos, 0, png->width * stride);
				break;
			case 3: /* average */
				if (outpos)
					png_filter_average(stride, filtered + pos, data + outpos,
					                   data + outpos - (png->width * stride), png->width * stride);
				else
					png_filter_average(stride, filtered + pos, data + outpos, 0,
					                   png->width * stride);
				break;
			case 4: /* paeth */
				if (outpos)
				...
				break;
			default: return PNG_UNKNOWN_FILTER;
		}
...
int png_set_data(png_t* png, unsigned width, unsigned height, char depth, int color,
	...
	for (unsigned int i = 0; i < png->height; i++) {
		...
		memcpy(&filtered[i * png->width * png->bpp + i + 1], data + i * png->width * png->bpp,
		       png->width * png->bpp);
	}
...
char* png_error_string(int error) {
	switch (error) {
		...
		case PNG_WRONG_ARGUMENTS:
			return "Wrong combination of arguments passed to png_open. You must use either a "
			       "read_function or supply a file pointer to use.";
