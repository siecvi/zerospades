...
#pragma mark - Real Vector
	...
	public:
		...
		Vector4 operator+(const Vector4& v) const { return Make(x + v.x, y + v.y, z + v.z, w + v.w); }
		Vector4 operator-(const Vector4& v) const { return Make(x - v.x, y - v.y, z - v.z, w - v.w); }
		Vector4 operator*(const Vector4& v) const { return Make(x * v.x, y * v.y, z * v.z, w * v.w); }
		Vector4 operator/(const Vector4& v) const { return Make(x / v.x, y / v.y, z / v.z, w / v.w); }
		...
		bool operator!=(const Vector4& v) const { return x != v.x || y != v.y || z != v.z || z != v.w; }
...
#pragma mark - Quaternion for Spatial Rotations
	...
	public:
		...
		inline Vector3 Apply(const Vector3& v) const {
			auto q = *this * Quaternion(v.x, v.y, v.z, 0.0F) * Conjugate();
		...
		static inline Quaternion DecodeRotation(Vector3 v) {
			...
			return Quaternion(v.x, v.y, v.z, sqrtf(1.0F - s));
		}
