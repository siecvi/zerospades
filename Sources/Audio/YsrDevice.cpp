...
extern int s_volumePrevious; // defined in ALDevice.cpp
...
namespace spades {
	namespace audio {
		...
		public:
			...
			struct InitParam {
				double samplingRate;
				void (*spatializerCallback)(const YVec3* origin, SpatializeResult*, void* userData);
		...
		YsrDevice::YsrDevice()
			...
			param.spatializerCallback = reinterpret_cast<void (*)(
			  const YsrContext::YVec3*, YsrContext::SpatializeResult*, void*)>(SpatializeCallback);
			...
			if (map == NULL) {
				...
				for (int rays = 0; rays < 4; rays++) {
					...
					if (hit) {
						...
						if (hit2)
							roomFeedbackHistory[roomHistoryPos] = 1.0F;
						else
							roomFeedbackHistory[roomHistoryPos] = 0.0F;
					}
