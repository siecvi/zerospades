...
int asCReader::ReadInner() 
{
	...
	module->enumTypes.Allocate(static_cast<asUINT>(count), false);
	...
	count = ReadEncodedUInt();
	module->classTypes.Allocate(static_cast<asUINT>(count), false);
	for( i = 0; i < count && !error; ++i )
	{
	...
	// Read func defs
	count = ReadEncodedUInt();
	module->funcDefs.Allocate(static_cast<asUINT> (count), false);
	for( i = 0; i < count && !error; i++ )
	{
		bool isNew;
	...
	for( asUINT i = 0; i < module->classTypes.GetLength() && !error; i++ )
	{
	...
	// Read typedefs
	count = ReadEncodedUInt();
	module->typeDefs.Allocate(static_cast<asUINT> (count), false);
	for( i = 0; i < count && !error; i++ )
	{
	...
	module->scriptGlobals.Allocate(static_cast<asUINT> (count), false);
	for( i = 0; i < count && !error; ++i ) 
	{
		ReadGlobalProperty();
	}
	...
	// bindInformations[]
	count = ReadEncodedUInt();
	module->bindInformations.Allocate(static_cast<asUINT> (count), false);
	for( i = 0; i < count && !error; ++i )
	{
	...
	// usedTypes[]
	count = ReadEncodedUInt();
	usedTypes.Allocate(static_cast<asUINT> (count), false);
	for( i = 0; i < count && !error; ++i )
	{
	...
	if( !error )
	{
		for( asUINT i = 0; i < usedTypes.GetLength() && !error; i++ )
		{
	...
	for( asUINT i = 0; i < module->scriptFunctions.GetLength() && !error; i++ )
