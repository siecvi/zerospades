...
#define Segment GLSSMRSegment
		...
		static Vector3 FrustrumCoord(const client::SceneDefinition& def, float x, float y, float z) {
			x *= z;
			y *= z;
...
#pragma mark - Sparse Processor
		...
		struct GLSparseShadowMapRenderer::Internal {
			...
			static Vector3 TileToShadowMapCoord(IntVector3 tile) {
				Vector3 v;
				v.x = (float)tile.x * (2 / (float)Tiles) - 1.0F;
				v.y = (float)tile.y * (2 / (float)Tiles) - 1.0F;
				return v;
			}
			LodUnit ComputeLod(Vector3 shadowCoord) {
				...
				float lod = 1.0F / (sqrtf(sq) + 0.01F);
				int iv = (int)(lod * 200.0F);
				if (iv < 1)
					iv = 1;
				if (iv > 65536)
					iv = 65535;
			...
			Internal(GLSparseShadowMapRenderer& r) : renderer(r) {
				...
				const auto& rmodels = r.GetRenderer().GetModelRenderer()->models;
				for (const auto& models : rmodels) {
					Instance inst;
					inst.model = models.model;
					OBB3 modelBounds = inst.model->GetBoundingBox();
					for (const auto& param : models.params) {
						inst.param = &param;
		...
		void GLSparseShadowMapRenderer::RenderShadowMapPass() {
			...
			Matrix4 baseMatrix = matrix;
			{
				...
				for (size_t i = 0; i < itnl.groups.size(); i++) {
					...
					Matrix4 mat;
					mat = Matrix4::Translate((dest1.x + dest2.x) * -0.5F, (dest1.y + dest2.y) * -0.5F, 0.0F);
					mat = Matrix4::Scale(2.0F / (dest2.x - dest1.x), 2.0F / (dest2.y - dest1.y), 1.0F) * mat;
					mat = mat * baseMatrix;
