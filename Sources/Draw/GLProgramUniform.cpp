...
namespace spades {
	namespace draw {
		GLProgramUniform::GLProgramUniform(const std::string& n) : name(n) { last = NULL; }
		...
		void GLProgramUniform::SetValue(IGLDevice::Float x, IGLDevice::Float y) {
			SPADES_MARK_FUNCTION_DEBUG();
		...
		void GLProgramUniform::SetValue(IGLDevice::Float x, IGLDevice::Float y, IGLDevice::Float z,
		                                IGLDevice::Float w) {
			SPADES_MARK_FUNCTION_DEBUG();
		...
		void GLProgramUniform::SetValue(IGLDevice::Integer x, IGLDevice::Integer y) {
			SPADES_MARK_FUNCTION_DEBUG();
			last->GetDevice()->Uniform(loc, x, y);
		}
		void GLProgramUniform::SetValue(IGLDevice::Integer x, IGLDevice::Integer y,
		...
		void GLProgramUniform::SetValue(IGLDevice::Integer x, IGLDevice::Integer y,
		                                IGLDevice::Integer z, IGLDevice::Integer w) {
			SPADES_MARK_FUNCTION_DEBUG();
			last->GetDevice()->Uniform(loc, x, y, z, w);
		}
		void GLProgramUniform::SetValue(const spades::Matrix4& m, bool transpose) {
			SPADES_MARK_FUNCTION_DEBUG();
