	...
	namespace draw {
					...
					for (int x = 0; x < fw; x += 4) {
						...
						for (int by = 0; by < 4; by++) {
							...
							for (int bx = 0; bx < 4; bx++) {
								...
								uint32_t v2 = (color & 0xFF00) * factor2;
								v1 += fog1 * factor;
								v2 += fog2 * factor;
								v1 &= 0xFF00FF00;
								v2 &= 0xFF0000;
				...
				for (int y = startY; y < endY; y += 4) {
					...
					for (int x = 0; x < fw; x += 4) {
						...
						for (int by = 0; by < 4; by++) {
							...
							dist = _mm_max_ps(dist, _mm_set1_ps(0.0F));
							dist = _mm_min_ps(dist, _mm_set1_ps(256.0F));
		...
		static uint32_t ConvertColor32(Vector4 col) {
			auto convertColor = [](float f) {
				...
				return static_cast<uint32_t>(Clamp(i, 0, 255));
			};
			uint32_t c;
			...
			// render debug lines
			{
				...
				for (size_t i = 0; i < debugLines.size(); i++) {
					...
					if (abs(x2 - x1) > abs(y2 - y1)) {
						...
						float depth = d1, ddepth = (d2 - d1) * fastRcp(x2 - x1 + 1.0F);
						if (x1 < 0) {
							long long v = dy;
							v *= -x1;
							cy += sgn * ((int)v / divisor);
							fract = v % divisor;
							depth -= x1 * ddepth;
						}
						fb += minX;
						...
						float depth = d1, ddepth = (d2 - d1) * fastRcp(y2 - y1 + 1.0F);
						if (y1 < 0) {
							long long v = dx;
							v *= -y1;
							cx += sgn * ((int)v / divisor);
							fract = v % divisor;
							depth -= x1 * ddepth;
						}
						fb += minY * fw;
