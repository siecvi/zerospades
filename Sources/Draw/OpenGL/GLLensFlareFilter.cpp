...
namespace spades {
	namespace draw {
		GLLensFlareFilter::GLLensFlareFilter(GLRenderer& renderer) : renderer(renderer) {
			...
			drawProgram = renderer.RegisterProgram("Shaders/OpenGL/LensFlare/Draw.program");
		...
		GLColorBuffer GLLensFlareFilter::Blur(GLColorBuffer buffer, float spread) {
			...
			blur_unitShift.SetValue(spread / (float)w, 0.0F);
			qr.Draw();
			buffer.Release();
			...
			blur_unitShift.SetValue(0.0F, spread / (float)h);
			qr.Draw();
			buf2.Release();
