...
namespace spades {
	namespace draw {
		...
		GLSoftLitSpriteRenderer::GLSoftLitSpriteRenderer(GLRenderer& renderer)
			...
			program = renderer.RegisterProgram("Shaders/OpenGL/SoftLitSprite.program");
		}
		...
		void GLSoftLitSpriteRenderer::Render() {
			...
			for (size_t j = 0; j < lights.size(); j++) {
				...
				float spotTan =
				  dl.type == client::DynamicLightTypeSpotlight ? tanf(dl.spotAngle * 0.5F) : 0.0F;
				for (size_t i = 0; i < sprites.size(); i++) {
					Sprite& spr = sprites[i];
					...
					if (dl.type == client::DynamicLightTypeSpotlight) {
						...
						if (forward > spr.radius) {
							...
							att *= 0.5F - (forward / spr.radius) * 0.5F;
						} else {
			...
			if (numLowResSprites > 0) {
				...
				// do gaussian blur
				GLProgram* program =
				  renderer.RegisterProgram("Shaders/OpenGL/PostFilters/Gauss1D.program");
				...
				// composite
				program = renderer.RegisterProgram("Shaders/OpenGL/PostFilters/PassThrough.program");
