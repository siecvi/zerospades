...
namespace spades {
	namespace draw {
		GLColorCorrectionFilter::GLColorCorrectionFilter(GLRenderer& renderer)
			...
			lens = renderer.RegisterProgram("Shaders/OpenGL/PostFilters/ColorCorrection.program");
		...
		GLColorBuffer GLColorCorrectionFilter::Filter(GLColorBuffer input, Vector3 tintVal,
			...
			float sharpeningFinalGainValue = Clamp((float)settings.r_sharpen, 0.0F, 1.0F);
			...
			if (sharpeningFinalGainValue > 0.0F) {
				...
				blur_unitShift.SetValue(1.0F / (float)input.GetWidth(), 0.0F);
				qr.Draw();
			}
			...
			if (settings.r_hdr) {
				...
				if (settings.r_bloom) {
					// make image sharper
					saturation.SetValue(0.85F * def.saturation * settings.r_saturation);
					enhancement.SetValue(0.7f);
				} else {
					saturation.SetValue(1.0F * def.saturation * settings.r_saturation);
