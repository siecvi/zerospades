...
namespace spades {
	namespace draw {
		GLBloomFilter::GLBloomFilter(GLRenderer &renderer) : renderer(renderer) {
			thru = renderer.RegisterProgram("Shaders/OpenGL/PostFilters/PassThrough.program");
		}
...
#define Level BloomLevel
		...
		GLColorBuffer GLBloomFilter::Filter(GLColorBuffer input) {
			...
			GLProgram *gammaMix = renderer.RegisterProgram("Shaders/OpenGL/PostFilters/GammaMix.program");
