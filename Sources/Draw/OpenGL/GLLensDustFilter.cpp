...
namespace spades {
	namespace draw {
		GLLensDustFilter::GLLensDustFilter(GLRenderer &renderer) : renderer(renderer) {
			thru = renderer.RegisterProgram("Shaders/OpenGL/PostFilters/PassThroughConstAlpha.program");
			gauss1d = renderer.RegisterProgram("Shaders/OpenGL/PostFilters/Gauss1D.program");
			dust = renderer.RegisterProgram("Shaders/OpenGL/PostFilters/LensDust.program");
...
#define Level GLLensDustFilterLevel
		...
		GLColorBuffer GLLensDustFilter::DownSample(GLColorBuffer tex, bool linearize) {
			...
			blur_texCoordOffsetUniform.SetValue(1.0F / w, 1.0F / h, -1.0F / w, -1.0F / h);

			blur_colorUniform(program);
			blur_colorUniform.SetValue(1.0F, 1.0F, 1.0F, 1.0F);

			blur_texCoordRangeUniform(program);
			blur_texCoordRangeUniform.SetValue(0.0F, 0.0F, (float)((w + 1) & ~1) / w,
			                                   (float)((h + 1) & ~1) / h);
		...
		GLColorBuffer GLLensDustFilter::GaussianBlur(GLColorBuffer tex, bool vertical) {
			...
			blur_unitShift.SetValue(vertical ? 0.0F : 1.0F / w, vertical ? 1.0F / h : 0.0F);
		...
		GLColorBuffer GLLensDustFilter::Filter(GLColorBuffer input) {
			...
			thru->Use();
			thruColor.SetValue(1.0F, 1.0F, 1.0F, 1.0F);
			...
			for (int i = (int)levels.size() - 1; i >= 1; i--) {
				...
				for (int j = 0; j < 1; j++) {
					...
					thruColor.SetValue(1.0F, 1.0F, 1.0F, 1.0F);
					thruTexCoordRange.SetValue(0.0F, 0.0F, 1.0F, 1.0F);
					dev.Enable(IGLDevice::Blend, false);
					qr.Draw();

					float cx = 0.f, cy = 0.0F;

					dev.BindTexture(IGLDevice::Texture2D, curLevel.GetTexture());
					thruColor.SetValue(1.0F, 1.0F, 1.0F, alpha);
					thruTexCoordRange.SetValue(cx, cy, 1.0F, 1.0F);
					dev.Enable(IGLDevice::Blend, true);
					qr.Draw();
			...
			dustNoiseTexCoordFactor.SetValue(facX, facY, facX / 128.0F, facY / 128.0F);
