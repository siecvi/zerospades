	...
	namespace draw {
		...
		namespace {
			...
			double GetWallClockTime() {
				auto duration = chrono::high_resolution_clock::now() - startTimePoint;
				return chrono::duration_cast<chrono::milliseconds>(duration).count() / 1000000.0;
			}
		} // namespace
		...
		void GLProfiler::BeginPhase(const std::string& name, const std::string& description) {
			...
			if (it == current.subphases.end()) // Create a new subphase
		...
		void GLProfiler::EndPhaseInner() {
			...
			current.measurementLatest.totalNumFrames++;
				...
				void Traverse(Phase& phase, int level) {
					...
					if (self.m_settings.r_debugTimingGPUTime) {
						std::snprintf(buf + indent, 511 - indent, "%s - %.3fms / %.3fms",
						              ...
						              result.totalGPUTime * 1000.0 * factor,
						              result.totalWallClockTime * 1000.0 * factor);
					} else {
		...
		void GLProfiler::DrawResult(Phase& root) {
			...
			struct ResultRenderer {
				...
				void Traverse(Phase& phase, int level) {
					...
					// draw bar
					float barScale = 4000.0F * self.m_settings.r_debugTimingOutputBarScale;
					...
					renderer.DrawImage(white, AABB2{cursor.x, cursor.y + 3.0F, boxWidth - childBoxWidth, 4.0F});
