...
#define CheckExistence(func)                                                                       \
	do {                                                                                           \
		...
		void SDLGLDevice::Enable(Enum s, bool b) {
		...
		const char* SDLGLDevice::GetString(Enum type) {
			SPADES_MARK_FUNCTION();
			switch (type) {
		...
		const char* SDLGLDevice::GetIndexedString(Enum type, UInteger index) {
			SPADES_MARK_FUNCTION();
			switch (type) {
		...
		GLenum SDLGLDevice::parseBlendEquation(Enum v) {
			SPADES_MARK_FUNCTION();
			switch (v) {
		...
		GLenum SDLGLDevice::parseBlendFunction(Enum v) {
			SPADES_MARK_FUNCTION();
			switch (v) {
		...
		void SDLGLDevice::BlendEquation(Enum mode) {
			CheckExistence(glBlendEquation);
			glBlendEquation(parseBlendEquation(mode));
			CheckError();
		}

		void SDLGLDevice::BlendEquation(Enum rgb, Enum alpha) {
		...
		GLenum SDLGLDevice::parseBufferTarget(Enum v) {
			SPADES_MARK_FUNCTION_DEBUG();
			switch (v) {
		...
		void SDLGLDevice::GenerateMipmap(Enum target) {
#if GLEW
			if (glGenerateMipmap)
		...
		IGLDevice::UInteger SDLGLDevice::CreateShader(Enum type) {
			...
			switch (type) {
				...
				case draw::IGLDevice::GeometryShader:
					ret = glCreateShader(GL_GEOMETRY_SHADER);
					break;
				default: SPInvalidEnum("type", type);
			}
#endif
		...
		IGLDevice::Enum SDLGLDevice::CheckFramebufferStatus(Enum target) {
			GLenum ret = 0;
#if GLEW
