...
namespace spades {
	namespace gui {
		...
		void StartupScreenHelper::ExamineSystem() {
			...
			Vector4 col = MakeVector4(1, 1, 1, 0.75F);
			Vector4 yellow = MakeVector4(1, 1, 0.5F, 1);
			Vector4 red = MakeVector4(1, 0.5F, 0.5F, 1);
			...
			for (const std::string& localeInfoName : localeDirectories) {
				...
				try {
					...
					SPLog("Error while reading the locale info for '%s': %s",
						locale.c_str(), ex.what());
				}
			}
			...
			if (numDisplayMode > 0) {
				...
				for (int i = 0; i < numDisplayMode; i++) {
					...
					modes.push_back(spades::MakeIntVector3(mode.w, mode.h, 0));
					SPLog("Video Mode Found: %dx%d", mode.w, mode.h);
				}
			} else {
				SPLog("Failed to get video mode list. Presetting default list");
				modes.push_back(spades::MakeIntVector3(640, 480, 0));
				modes.push_back(spades::MakeIntVector3(800, 600, 0));
				modes.push_back(spades::MakeIntVector3(1024, 768, 0));
				modes.push_back(spades::MakeIntVector3(1280, 720, 0));
				modes.push_back(spades::MakeIntVector3(1920, 1080, 0));
			}
			...
			if (!context) { // OpenGL initialization failed!
				...
				AddReport();
				AddReport("Message from SDL:", col);
				AddReport(err, col);
				...
				for (size_t i = 0; requiredExtensions[i]; i++) {
					...
					if (extensions.find(ex) == std::string::npos) {
						// extension not found
						AddReport(std::string(ex) + " is NOT SUPPORTED!", red);
						capable = false;
					}
				}
